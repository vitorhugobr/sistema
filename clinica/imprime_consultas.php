...
class PDF extends FPDF
{
  function Header(){ //CABECALHO
	...
	$titulo_rel = "Lista Espera ".$clinica." em ".$data;
	$this->Cell(0,0,utf8_decode("Consultas ".$clinica." em ".$data),0,0,'C'); // CRIA UMA CELULA COM OS SEGUINTES DADOS, RESPECTIVAMENTE: 

	  //FINAL DO CABECALHO COM DADOS
	$this->SetXY(8,23); // SetXY -> DEFINE O X E O Y NA PAGINA
	$this->SetFont('Courier','',10);
	$this->Cell(0,1,str_pad(utf8_decode("Cód. Cad."),8)." ".str_pad("Nome", 55 , " ",STR_PAD_RIGHT),0,0,'L') ;
...
$pdf->SetTitle(utf8_decode('Consultas '.$clinica));
...
if($total==0){
	...
	while($dado = $statement->fetch()) {	// ENQUANTO OS DADOS VÃO PASSANDO, O FPDF VAI INSERINDO OS DADOS NA PAGINA			
		...
		$nome = $dado['nome']; 
		$nome = str_pad($nome, 58 , " ",STR_PAD_RIGHT);
		$sexo = $dado['sexo']; 
		$sexo = str_pad($sexo, 6 , " ",STR_PAD_BOTH);		
		$dtnasc = $dado['dtnasc']; 
		$dtnasc = FormatDateTime($dado['dtnasc'],7);
		$dtnasc = str_pad($dtnasc, 12 , " ",STR_PAD_LEFT);		
		$pdf->SetXY(1,$linha);
		if (!$zebrado){
		...
		$pdf->Cell(190,5,str_pad($cod_cadastro,11," ",STR_PAD_LEFT)." ".utf8_decode($nome),0,0,'L') ;
		$linha = $linha +5;
		}
	}
